...
def load_constraint(save_dir, args):
    """Load constraint from save_dir"""
    ...
    actor.load_state_dict(torch.load(f"{save_dir}/actor.pt"))
    safety_threshold = 0.5 / (1 - args.gamma)
    return actor, qf1, qf2, safety_threshold


def get_constrained_action(
    ...
    """Return action that satisfies the safety constraint"""
...
def add_constrain_train_args(
    parser: argparse.ArgumentParser,
) -> argparse.ArgumentParser:
    # TODO: implement
    parser.add_argument("--constraint_dir", type=str, default="downloads/1qxv7iyl")
    parser.add_argument(
        "--constrain",
...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
        ...
        if args.constrain:
            actions = np.array(
                [get_constrained_action(*constraint, obs[0], actions[0])]
            )
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % 100 == 0:
                ...
                writer.add_scalar(
                    "charts/safety_violations", safety_violations, global_step
                )
